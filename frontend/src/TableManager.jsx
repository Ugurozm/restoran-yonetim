...
export default function TableManager() {
  ...
  async function handleAddTable() {
    if (!newNumber) return alert('Masa numarasi girin')
    setLoading(true)
    try {
  ...
  return (
    <div style={{ padding: '32px' }}>
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ color: '#9B7B6A', fontSize: '14px' }}>{tables.length} masa tanimli</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '28px' }}>
        {[
          ...
          { label: 'Bos', value: stats.empty, color: '#1E7E34', bg: '#EDF7F0' },
      ...
      <div style={{ background: '#fff', borderRadius: '20px', padding: '20px 24px', marginBottom: '24px', boxShadow: '0 2px 12px rgba(44,26,14,0.07)' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <input type="number" placeholder="Masa numarasi" value={newNumber}
          ...
          <div style={{ background: '#fff', borderRadius: '20px', padding: '24px', boxShadow: '0 2px 16px rgba(44,26,14,0.08)', height: 'fit-content', position: 'sticky', top: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <button onClick={() => { setSelected(null); setConfirmAction(null) }}
                style={{ background: '#F5F0EB', color: '#9B7B6A', padding: '6px 12px', borderRadius: '8px', fontSize: '13px' }}>✕</button>
            </div>
              ...
              {qrMap[selected.table_number] ? (
                <>
                  ...
                  <div style={{ fontSize: '12px', color: '#9B7B6A', marginTop: '10px' }}>
                    {window.location.origin}/masa/{selected.table_number}
                  </div>
                </>
              ) : (
                <div style={{ padding: '30px', color: '#9B7B6A', fontSize: '14px' }}>Yukleniyor...</div>
              )}
            </div>
            ...
            {!confirmAction ? (
              ...
              <div style={{ background: '#FDF6E8', borderRadius: '14px', padding: '16px' }}>
                <div style={{ fontSize: '14px', color: '#8B6914', marginBottom: '14px', lineHeight: 1.5 }}>
                  {confirmAction === 'open' && `Masa ${selected.table_number} icin manuel hesap acilacak. Emin misiniz?`}
                  {confirmAction === 'close' && `Masa ${selected.table_number} hesabi kapatilacak. Emin misiniz?`}
                  {confirmAction === 'delete' && `Masa ${selected.table_number} silinecek. Bu islem geri alinamaz!`}
                </div>
